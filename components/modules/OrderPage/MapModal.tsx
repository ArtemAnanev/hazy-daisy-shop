...
import {
  setChosenCourierAddressData, setCourierAddressData, setShouldLoadHazyData, setShouldShowCourierAddressData
} from "@/context/order"
import { useUnit } from 'effector-react'
import {
  $chosenPickupAddressData, $courierAddressData,
  $hazyDataByCity,
  $mapInstance, $shouldShowCourierAddressData
...
import CourierAddressesItem from "@/components/modules/OrderPage/CourierAddressesItem"
...
const MapModal = () => {
  ...
  const drawMarkerByClick = async (e) => {
    ...
    if (result) {
      removeMapMarkers()
      await drawMarker(e.lngLat.lng, e.lngLat.lat, ttMapInstance)
  ...
  const handleLoadMap = async (initialContainer = pickUpMapRef) => {
    ...
    const setMarkersByLocationsData = (data: IHazyAddressData[]) => {
      data.forEach((item) => {
    ...
    if (!!chosenPickupAddressData.address_line1) {
      const chosenItem = hazyDataByCity.filter(
    ...
    if (!userGeolocation.features) {
      const data = await handleSelectPickupAddress('Нахабино')

      setMarkersByLocationsData(data)
      ttSearchBox.setValue('Нахабино')
    } else {
      map
        .setCenter([
    ...
    if (hazyDataByCity.length) {
      setMarkersByLocationsData(hazyDataByCity)
    }
