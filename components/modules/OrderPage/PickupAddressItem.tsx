...
const PickupAddressItem = ({ addressItem, handleChosenAddressData, handleSelectAddress }: IPickupAddressItemProps) => {
  const selectAddress = () => {
  ...
  return (
    <li className={styles.order__list__item__delivery__list__item}>
      <button className='btn-reset' onClick={selectAddress}>
        ...
        <span>{addressItem.address_line2}, {addressItem.city}</span>
      </button>
    </li>
  )
}
