...
const PromotionalCode = ({setIsCorrectPromotionalCode}:{setIsCorrectPromotionalCode: (arg0: boolean)=> void
}) => {
  const [value, setValue] = useState('')
  const isCorrectCode = value === "MARGOSHA"
  const { lang, translations } = useLang()

  const handleChangeValue = ( e: React.ChangeEvent<HTMLInputElement> ) => {
    setValue(e.target.value)
    if (e.target.value === 'MARGOSHA') {
  ...
  return (
    <div className={styles.cart__promotional_code}>
      <input
        ...
        style={isCorrectCode ? { border: '1px solid #16D9A6', color: '16D9A6' }: {}
      }
      />
